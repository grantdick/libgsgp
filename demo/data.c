               ...
               bool standardise, double **mean, double **sd)
{
    ...
    *testY = Y;
    ...
    if (standardise) {
        *mean = col_means(*trainX, *trainY, *n_train, *n_features);
        *sd   = col_sd(*trainX, *trainY, *n_train, *n_features);

        for (i = 0; i < *n_features; ++i) {
            for (j = 0; j < *n_train; ++j) (*trainX)[j][i] = ((*trainX)[j][i] - (*mean)[i]) / (*sd)[i];
            for (j = 0; j < *n_test; ++j)  (*testX)[j][i] = ((*testX)[j][i] - (*mean)[i]) / (*sd)[i];
        }

        for (j = 0; j < *n_train; ++j) (*trainY)[j] = ((*trainY)[j] - (*mean)[*n_features]) / (*sd)[*n_features];
        for (j = 0; j < *n_test; ++j)  (*testY)[j] = ((*testY)[j] - (*mean)[*n_features]) / (*sd)[*n_features];
    } else {
        if (mean) {
            *mean = malloc((*n_features + 1) * sizeof(double));
            for (i = 0; i <= *n_features; ++i) (*mean)[i] = 0;
        }

        if (sd) {
            *sd   = malloc((*n_features + 1) * sizeof(double));
            for (i = 0; i <= *n_features; ++i)   (*sd)[i] = 1;
        }
    }

    if (bounds) {
        B = malloc(*n_features * sizeof(double *));
        B[0] = malloc(2 * *n_features * sizeof(double));
        for (i = 1; i < *n_features; ++i) B[i] = B[0] + (2 * i);

        for (i = 0; i < *n_features; ++i) {
            B[i][0] = B[i][1] = (*trainX)[0][i];
            for (j = 1; j < *n_train; ++j) {
                if (B[i][0] > (*trainX)[j][i]) B[i][0] = (*trainX)[j][i];
                if (B[i][1] < (*trainX)[j][i]) B[i][1] = (*trainX)[j][i];
            }
        }
