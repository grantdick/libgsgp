                             ...
                             struct gsgp_parameters *param, struct gsgp_archive *archive, void *args)
{
    ...
    fprintf(stdout, "%4d %10f %10f %10f %10f %10f %4d\n", g,
...
static void process_parameter(char *param, struct gsgp_parameters *params, struct data_set_details *details)
{
    char *line, *key, *value;
    ...
    } else if (strncmp(key, "logistic_mutation", 17) == 0) {
        params->logistic_mutation = strncmp(value, "Y", 1) == 0;
    ...
    } else if (strncmp(key, "crossover_coef", 14) == 0) {
        if (strncmp(value, "FIXED", 5) == 0) {
            params->crossover_coef = FIXED_PROPORTION;
        } else if (strncmp(value, "RANDOM", 6) == 0) {
            params->crossover_coef = RANDOM_PROPORTION;
        } else if (strncmp(value, "LEAST_SQUARES", 13) == 0) {
            params->crossover_coef = LEAST_SQUARES_CROSSOVER;
        } else if (strncmp(value, "MULTIPLE_LEAST_SQUARES", 22) == 0) {
            params->crossover_coef = MULTIPLE_LEAST_SQUARES_CROSSOVER;
        } else {
            fprintf(stderr, "ERROR: Unknown value for parameter crossover_type: %s\n", value);
            exit(EXIT_FAILURE);
        }
    } else if (strncmp(key, "mutation_coef", 13) == 0) {
        if (strncmp(value, "FIXED", 5) == 0) {
            params->mutation_coef = FIXED_SCALE;
        } else if (strncmp(value, "RANDOM", 6) == 0) {
            params->mutation_coef = RANDOM_SCALE;
        } else if (strncmp(value, "LEAST_SQUARES", 13) == 0) {
            params->mutation_coef = LEAST_SQUARES_MUTATION;
        } else {
...
static void parse_parameters(char *params_file, struct gsgp_parameters *params, struct data_set_details *details)
{
    ...
    while (!feof(input)) {
        if (strlen(line) > 0) {
            process_parameter(line, params, details);
        }
        line = next_line(&buffer, &bufsz, input);
    }
    fclose(input);
...
int main(int argc, char **argv)
{
    ...
    i = 4; while (i < argc) {
        if (strncmp(argv[i], "-p", 2) == 0) {
            process_parameter(argv[i + 1], params, &details);
            i += 2;
        } else {
            parse_parameters(argv[i++], params, &details);
        }
    }
